...
function ElapsedTime() {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      setElapsed(h > 0 ? `${h} hour${h > 1 ? "s" : ""} ago` : m > 0 ? `${m} min${m > 1 ? "s" : ""} ago` : `${s} sec${s > 1 ? "s" : ""} ago`)
    }, 10000)
...
export default function SetupPage() {
  ...
  const setCompleted = (id: Step["id"], done = true) => {
    setSteps((prev) => prev.map((s) => (s.id === id ? { ...s, completed: done } : s)))
  }
    ...
    try {
      ...
      for (const item of json) {
        ...
        valid.push({
          ...
          tags: Array.isArray(item.tags) ? item.tags.map((t: string) => t.trim()).filter(Boolean) : undefined,
        })
      }
  ...
  const employees = [
    { id: "as", name: "Atharv Sharma", role: "Team Admin" },
            ...
            <div className="space-y-4">
                ...
                >
                  <div
                    className={`flex-shrink-0 w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all ${
                      steps[0].completed ? "bg-primary border-primary" : "border-muted-foreground hover:border-primary"
                    }`}
                  >
                    {steps[0].completed && (
                      <svg className="w-4 h-4 text-primary-foreground" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" />
                      </svg>
                    )}
                  </div>
                  <div className="flex-1">
                    <p className={`font-medium ${steps[0].completed ? "text-muted-foreground line-through" : ""}`}>
                      1.&nbsp;{steps[0].title}
                    </p>
                  </div>
                  <svg
                ...
                {openStep === 1 && (
                  <div className="px-4 pb-4 space-y-3" onClick={(e) => e.stopPropagation()}>
                    <div className="rounded-lg border-2 border-dashed p-6 text-center">
                      ...
                      />
                      <p className="mt-2 text-xs text-muted-foreground">  Upload JSON array: [<code>{'{ question, answer, tags? }'}</code>]</p>
                    </div>

                    {parsedFaqs.length > 0 && (
                      <div className="space-y-2">
                        <div className="text-sm font-medium">Preview ({parsedFaqs.length} items)</div>
                            ...
                            <tbody>
                              {parsedFaqs.map((faq, i) => (
                                <tr key={i} className="border-t">
                                  <td className="p-2 max-w-xs truncate">{faq.question}</td>
                                  <td className="p-2 max-w-xs truncate">{faq.answer}</td>
                                  <td className="p-2">{faq.tags?.join(', ') || '—'}</td>
                                </tr>
                              ))}
                            </tbody>
                          </table>
                        ...
                        {importResults.length > 0 && (
                          <div className="text-sm space-y-1">
                            <p className="font-medium">
                              Progress: {importResults.filter((r) => r.success).length}/{importResults.length} successful
                            </p>
                            {importResults.some((r) => !r.success) && (
                              <div className="text-destructive text-xs max-h-20 overflow-y-auto">
                                {importResults
                                  ...
                                  .map((r, i) => (
                                    <div key={i}>
                                      Failed: {r.item.question.slice(0, 50)}... ({r.error})
                                    </div>
                                  ))}
                              </div>
                            )}
              ...
              <div className="rounded-lg border-2 hover:border-primary/50 transition-all">
                ...
                >
                  <div
                    className={`flex-shrink-0 w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all ${
                      steps[1].completed ? "bg-primary border-primary" : "border-muted-foreground hover:border-primary"
                    }`}
                  >
                    {steps[1].completed && (
                      <svg className="w-4 h-4 text-primary-foreground" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                  ...
                  <div className="flex-1">
                    <p className={`font-medium ${steps[1].completed ? "text-muted-foreground line-through" : ""}`}>
                      2.&nbsp;{steps[1].title}
                    </p>
                    {selectedEmployee && (
                      <p className="text-sm text-muted-foreground mt-1">
                        Selected: <span className="font-medium">{employees.find((e) => e.id === selectedEmployee)?.name}</span>
                      </p>
                    )}
                  </div>
                  <svg
                ...
                {openStep === 2 && (
                  <div className="px-4 pb-4" onClick={(e) => e.stopPropagation()}>
                    <label className="text-sm text-muted-foreground block mb-2">Mock user for testing</label>
                    <div className="flex items-center gap-3">
                      ...
                      >
                        <option value="" disabled>
                          {creatingSession ? 'Creating session...' : 'Pick a mock user…'}
                        </option>
                        {employees.map((e) => (
                      ...
                      <span className="text-xs text-muted-foreground">Creates session automatically.</span>
                    </div>
                    {sessionId && (
                      <p className="text-xs text-muted-foreground mt-2">Session created: {sessionId}</p>
                    )}
                  </div>
                )}
              </div>
              ...
              <div className="rounded-lg border-2 hover:border-primary/50 transition-all">
                ...
                >
                  <div
                    className={`flex-shrink-0 w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all ${
                      steps[2].completed ? "bg-primary border-primary" : "border-muted-foreground hover:border-primary"
                    }`}
                  >
                    {steps[2].completed && (
                      <svg className="w-4 h-4 text-primary-foreground" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                  ...
                  <div className="flex-1">
                    <p className={`font-medium ${steps[2].completed ? "text-muted-foreground line-through" : ""}`}>
                      3.&nbsp;{steps[2].title}
                    </p>
                  </div>
                  <svg
                  ...
                  <div className="px-4 pb-4" onClick={(e) => e.stopPropagation()}>
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-xs text-muted-foreground">
                        {sessionId ? 'Opens in a new tab.' : 'Complete Step 2 first.'}
                      </span>
                    </div>
                  </div>
                )}
